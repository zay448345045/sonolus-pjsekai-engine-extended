...
export const createTimeToScaledTime =
    (archetypes: Archetypes, bpmChanges: BpmChanges) =>
    (baseTime: number, tsGroup: number, noCache?: boolean): number => {
...
export const createScaledTimeToEarliestTime =
    (archetypes: Archetypes, bpmChanges: BpmChanges) =>
    (time: number, tsGroup: number): number => {
        // if (tsGroup === 0) {
        //     return time
        // }
        const tsGroupEntity = archetypes.TimeScaleGroup.data.get(tsGroup)
